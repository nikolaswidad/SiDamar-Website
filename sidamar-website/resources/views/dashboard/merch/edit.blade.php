...
<h1 class="text-4xl font-semibold font-montserrat">Ubah Event Donasi</h1>
    ...
    <!-- Edit Modal -->
    <div class="modal fade" id="editModal-{{ $merch->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="max-w-5xl">
                    ...
                    <form action="{{ route('merch.update', $merch->id) }}" method="POST" class="max-w-3xl mt-8 mb-8 font-montserrat">
                        ...
                        {{-- <div class="form-group mb-6 col-span-6">
                            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="image">Upload Image</label>
                              <input class="form-control block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 @error('image') is-invalid @enderror" name="image" id="image" type="file" onchange="previewImage()" value="{{ $merch->image }}">
                            @error('image')
                                  {{ $message }}
                            @enderror
                        </div> --}}
